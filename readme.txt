...
　　　・MetaAI,NavigationAI,CharacterAIのソースファイルと、それを作成しているMainGameSceneのInitialize関数のコード
	理由：ナビAIだけにすべての情報を持たせて、そこからメタAIやキャラAIが情報を貰ったり判断するようにさせてそれぞれのAIの役割をはっきりさせたつもりなのですが、
	　ナビAIに一々情報を聞かなければならず、メタAIの処理が遠回りになってしまっている気がするので、見てもらいたいです。

　　　・キャラクターの攻撃処理（Character.cppの39行目で定義、CharacterAttackStateでコリジョンを付与、除外）
	理由：現在はキャラクタークラスで攻撃判定のコリジョンを宣言しておいて、攻撃する時だけそのコリジョンを追加して終わったら除外すると言う方法で攻撃をしているのですが、
	　今の方法だとRelease処理の中でキャラクターのstate毎に開放処理を変えなければ、多重開放で例外が出てしまうのでそもそもこの攻撃の仕方が最適なのか見てもらいたいです。
